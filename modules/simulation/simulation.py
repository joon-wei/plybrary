...
def add_long_sltp_fees_graph(df, trade_size, trade_start, stop_loss=1, take_profit=1, leverage=1, slippage=False ,fee=0.0005):
    ...
    for date in df.loc[trade_start:].index:
        if trade_active:
            ...
            if low_price <= stop_loss_price:    # short change here
            ...
            elif high_price >= take_profit_price:    # short change here
    ...
    mpf.plot(
        ...
        type='candle',
        style='yahoo',
        addplot=[capital_plot, stop_loss_plot, take_profit_plot],
        figsize=(25, 15),
...
def add_short_sltp_fees_graph(df, trade_size, trade_start, stop_loss=1, take_profit=1, leverage=1, slippage=False, fee=0.0005):
    ...
    for date in df.loc[trade_start:].index:
        if trade_active:
            ...
            high_price = df.loc[date, 'High']
            low_price = df.loc[date, 'Low']
            if high_price >= stop_loss_price:    # short change here
                df.loc[date, 'exit_reason'] = 'Stop-Loss Triggered'
                df.loc[date, 'trade_position'] = trade_size * leverage
                df.loc[date,'trade_notional'] = units * df.loc[date,'Close']
                df.loc[date,'taker_fee_1'] = df.loc[date,'trade_position'] * fee
            ...
            elif low_price <= take_profit_price:    # short change here
                df.loc[date, 'exit_reason'] = 'Take-Profit Reached'
                df.loc[date, 'trade_position'] = trade_size * leverage
                df.loc[date,'trade_notional'] = units * df.loc[date,'Close']
                df.loc[date,'taker_fee_1'] = df.loc[date,'trade_position'] * fee
